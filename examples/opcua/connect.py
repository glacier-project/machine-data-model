"""
Example: Connects to the iot-edge-opc-plc OPC UA server.
...
from machine_data_model.nodes.connectors.abstract_connector import SubscriptionArguments, AbstractConnector
...
def read_and_write_variable_node_using_connector(connector: AbstractConnector, node_path: str):
    """
...
def subscribe_to_variable_node_using_connector(connector: AbstractConnector, node_path: str):
    """
...
def main() -> None:
    ...
    try:
        ...
        print("Call method with no inputs, using a data model node with 'remote_path' attribute:")
        output_method_objects = "Objects/Methods_Output_With_Remote_Path"
        call_method(data_model, output_method_objects, [])
        print("----------------\n")

        print("Call method with no inputs, using a data model node with 'node_id' attribute:")
    ...
    print("Waiting 10 seconds to allow subscriptions to receive changing values")
    time.sleep(10)
    print("----------------\n")

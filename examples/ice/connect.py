...
SELECTED_CONFIG = "conveyor-opcua.yml"  # Options: Cell4-opcua.yml, Cell5-opcua.yml, conveyor-opcua.yml
...
def inspect_data_model(data_model) -> None:
    ...
    print(f"{'-'*60}")
    print(f"  Name: {data_model.name if hasattr(data_model, 'name') else 'N/A'}")
    print(f"  Root: {data_model.root if hasattr(data_model, 'root') else 'N/A'}")
...
def call_conveyor_method(data_model) -> None:
    ...
    print(f"  Parameters: pallet_id=1, destination=15")
    ...
    try:
        ...
        print(f"\n✗ Node not found: {e}")
        print(f"  Check that the node path exists in the data model")
...
def main() -> None:
    ...
    try:
        ...
        if SELECTED_CONFIG == "conveyor-opcua.yml":
            ...
            print(f"\n{'-'*60}")
            print("NOTE: Method call example is configured for conveyor-opcua.yml")
            print("      Modify the example to call methods for other configurations")
            print(f"{'-'*60}")
